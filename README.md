...
One Node process, the supervisor, holds the live terminals -- one pi (or other program) per PTY, kept running in the background once you've joined it, whether or not anyone is looking. Each session is a real terminal (a PTY) of its own, and every viewer -- phone, desktop tab, any number of `attach`es -- is a WebSocket client watching one of them. A session's own grid is one size for everyone watching it: the narrowest viewer's, because a real terminal can only be one width.
...
Optional: install the pi extension to show a `mobile-tty` indicator in pi's footer and print the model at the bottom of the phone screen (otherwise it is obscured at narrow widths). The extension is only active when pi is run through mobile-tty.
```
...
The `≡` menu lists pi's saved conversations, running ones first and then newest first, with a live terminal marked ●. A session pi forked off another (a subagent's run) belongs to the session that spawned it and is no row of its own. Tap one to join it -- if no terminal has it open, a pi process starts in the background; otherwise you join the existing process, exactly as it was left. Pi's `/new` and `/resume` change the conversation in that terminal without changing which process viewers are attached to. Joining never ends anything else: leave a session and it keeps running, so the phone, a browser tab and any number of `attach`ed terminals can each be looking at a different one, the same as running pi a few times in different terminals -- except the menu is how you get back to any of them from the phone.
...
- A folder used for several separate pi conversations offers all of them in the list, not just the newest. `/resume` inside pi works too: the live marker follows the conversation while viewers remain attached to that pi process.
...
One node process, the supervisor, owns the front door; each live terminal it holds is a *second* node process, one per pi (or other program), spawned over a Unix socket rather than a network port. A viewer -- phone, desktop tab, `attach` -- connects to a stable PTY ID (`/ws?process=<id>`), while a saved conversation is selected by its pi ID and folder. The supervisor sends the PTY ID on admission, then forwards terminal bytes unchanged.
...
- **`server/registry.js`** spawns and tracks background PTYs by their own IDs -- each is `server/cli.js` re-invoked with `--internal-socket`, so it's exactly `server/index.js` bound to a Unix socket instead of a port. `pi-extensions/mtty-session.ts` reports which conversation a pi process currently has open. Past a cap (4 by default) a new session ends the one written longest ago to make room -- its transcript and its subagents' forks both counting -- but only one nobody is watching and that pi has left alone for ten minutes. When nothing qualifies, the new session is refused rather than anything being ended behind its owner's back.
