...
- **`server/registry.js`** spawns and tracks background PTYs by their own IDs -- each is `server/cli.js` re-invoked with `--internal-socket`, so it's exactly `server/index.js` bound to a Unix socket instead of a port. `pi-extensions/mtty-session.ts` reports which conversation a pi process currently has open. Past a cap (4 by default) a new session ends the least recently touched one to make room -- but only one nobody is watching and that pi, or any subagent it forked work to, has left alone for ten minutes. When nothing qualifies, the new session is refused rather than anything being ended behind its owner's back.
