...
- **A session outlives the viewer that joined it, on purpose**, and the pool is bounded (`cap`, default 4) rather than left to grow forever. Past the cap, a new session ends the least recently touched one -- but only one nobody is watching and that has been quiet for ten minutes by every sign there is: its PTY, its transcript, and the forked sessions of any subagent it handed work to. A session failing any of those is never ended behind its owner's back, and a start that finds nothing safe to end is refused instead. A restart of the supervisor still ends every session at once -- no adoption or reattachment across it was built, since that traded a meaningful amount of complexity (persisted socket tables, orphan detection) for a case (the server process itself restarting) that was already destructive before this existed.
